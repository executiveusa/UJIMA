...
const starter = [
  { role: 'assistant', content: 'What do you want UJIMA to work on?' },
];
...
export default function LabPage() {
  ...
  async function submit(event) {
    ...
    try {
      await supabaseRest('ujima_messages', {
        method: 'POST',
      ...
      await supabaseRest('ujima_messages', {
        method: 'POST',
        body: JSON.stringify({ conversation_id: conversationId, role: 'assistant', content: data.text, route: data.route || 'demo' }),
      });
      await supabaseRest('ujima_model_runs', {
        method: 'POST',
        body: JSON.stringify({ user_id: user.id, conversation_id: conversationId, route: data.route || 'demo', provider: data.provider || 'local', model: data.model || 'deterministic-demo', status: 'completed' }),
      });
      await supabaseRest(`ujima_conversations?id=eq.${conversationId}`, {
        method: 'PATCH',
