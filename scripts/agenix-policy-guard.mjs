...
const evidenceRequiredActions = new Set([
  'verify_live_post',
  'reconcile_evidence',
  'attach_evidence'
]);
...
function emit(result) {
  if (request.json) {
    ...
    if (result.evidence_id) console.log(`evidence_id: ${result.evidence_id}`);
  } else {
...
if (evidenceRequiredActions.has(request.action) && !request.evidenceId) {
  deny('Verification/evidence action requires a recorded evidence ID.');
}
